...
export function HistoryPagination({ totalPages, currentPage }: Props) {
  const router = useRouter()
  const searchParams = useSearchParams()

  const handlePageChange = (page: number) => {
    // Clona los parámetros actuales (filtros, fechas) para no perderlos
    ...
    <div className="flex items-center justify-center gap-2 p-4 mt-4 border-t border-zinc-200">
        ...
        <div className="flex items-center gap-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                // Lógica para mostrar solo algunas páginas si son muchas (opcional simple)
                // Aquí mostramos todas si son pocas, o podrías limitar el rango.
                // Para mantenerlo simple y funcional como el inventario:
